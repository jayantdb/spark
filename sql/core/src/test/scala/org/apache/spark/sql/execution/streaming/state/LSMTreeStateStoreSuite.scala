...
class LSMTreeStateStoreSuite extends SparkFunSuite with BeforeAndAfter {
  ...
  test("prefix scan") {
    ...
    try {
      ...
      val store2 = provider.getStore(1)
      // prefix scan on integers may not be meaningful,
  ...
  test("binary key ordering - iterator returns keys in lexicographic order") {
    ...
    try {
      ...
      assert(retrievedKeys == retrievedKeys.sorted, s"Keys not in sorted order: $retrievedKeys")
  ...
  test("binary key ordering - prefix ordering (shorter first)") {
    ...
    assert(keys == List("ab", "abc", "abcd"), s"Keys not in prefix order: $keys")
  }
  ...
  test("binary key ordering - long keys with word-at-a-time comparison") {
    ...
    assert(compareByteArrays(highByte1, highByte2) < 0, "0x7F should be less than 0x80 (unsigned)")
    assert(compareByteArrays(highByte2, highByte3) < 0, "0x80 should be less than 0xFF (unsigned)")
  }

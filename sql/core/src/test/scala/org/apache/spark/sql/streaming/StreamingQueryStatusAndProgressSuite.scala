...
//  }
  ...
  test("SPARK-53491: `test new code`") {
    val resourceUrl = getClass.getResource("/structured-streaming/sample-data-files")
...
//      assert(progress.inputBytesRead != null)
  ...
  test("SPARK-53491: `test new code - individual sources`") {
    val resourceUrl1 = getClass.getResource("/structured-streaming/sample-files-1")
    val resourceUrl2 = getClass.getResource("/structured-streaming/sample-files-2")
//    assert(resourceUrl != null, "Resource file not found!")

    val df1 = spark.readStream.textFile(resourceUrl1.getPath)
    val df2 = spark.readStream.textFile(resourceUrl2.getPath)
    val query = df1.union(df2).writeStream
      .format("memory")
      .queryName("TestInputBytes_2")
      .trigger(Trigger.AvailableNow)
      .start()
